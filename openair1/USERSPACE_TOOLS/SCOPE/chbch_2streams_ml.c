...
void chbch_scope_idle_callback(void)
{
  ...
  float Re,Im,mag_sig[NB_ANTENNAS_RX*NUMBER_OF_OFDM_CARRIERS*NUMBER_OF_OFDM_SYMBOLS_PER_SLOT*8],mag_h[NB_ANTENNAS_RX*NUMBER_OF_OFDM_CARRIERS*NUMBER_OF_OFDM_SYMBOLS_PER_SLOT*8],
        mag_sig2[NUMBER_OF_OFDM_CARRIERS*NUMBER_OF_OFDM_SYMBOLS_PER_SLOT*8],sig_time[NB_ANTENNAS_RX*NUMBER_OF_OFDM_CARRIERS*NUMBER_OF_OFDM_SYMBOLS_PER_SLOT*8],
        scat_sig_re[NUMBER_OF_OFDM_CARRIERS*NUMBER_OF_OFDM_SYMBOLS_PER_SLOT*8],scat_sig_im[NUMBER_OF_OFDM_CARRIERS*NUMBER_OF_OFDM_SYMBOLS_PER_SLOT*8],
        scat_sig_re2[NUMBER_OF_OFDM_CARRIERS*NUMBER_OF_OFDM_SYMBOLS_PER_SLOT*8],scat_sig_im2[NUMBER_OF_OFDM_CARRIERS*NUMBER_OF_OFDM_SYMBOLS_PER_SLOT*8];
  ...
  if (sach_flag==0) // CHBCH or MRBCH
    for (i=0; i<256*length; i+=2) {
      ...
      sig_time[j] = (float)j;
      mag_sig[j++] = (float)(rx_sig_f2[1][2*i]);
      sig_time[j] = (float)j;
      mag_sig[j++] = (float)(rx_sig_f2[1][1+(2*i)]);
  ...
  if (sach_flag == 0)
    for (i=10; i<256*length; i+=2) {

      if ((i%256) == 80)  //skip the zero carriers
        i+=96;

      if ((i%256) == 246) //skip the DC
        i+=20;

      Re = (float)(rx_sig_f2[1][2*i]);
      Im = (float)(rx_sig_f2[1][1+(2*i)]);

      scat_sig_re[j] = Re;
      scat_sig_im[j] = Im;

      Re = (float)(rx_sig_f2[1][2*(i+1)]);
      Im = (float)(rx_sig_f2[1][1+(2*(i+1))]);
