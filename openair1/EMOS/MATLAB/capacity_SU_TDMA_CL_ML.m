function [CAP] = capacity_SU_TDMA_CL_ML(H,SNR,normalize)
%
% [CAP] = capacity_SU_TDMA_ML(H,SNR,normalize)
%
%  This function calculates the multiuser (MU) capacity of the channel
%  assuming a time-shared single user (SU) MIMO tranmission and a ML receiver.  
...
CAP_SU = zeros(NUser,Nframes*NFreq,N_SNR);

for k=1:NUser
    CAP_SU(k,:,:) = capacity_SU_CL_ML(H(:,:,:,:,k),SNR,normalize);
end
%CAP = mean(CAP_SU,2);
CAP = reshape(CAP_SU,NUser*Nframes*NFreq,N_SNR);
