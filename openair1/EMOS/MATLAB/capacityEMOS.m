function [CAP] = capacityEMOS(HH,SNR,normalize)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% [CAP] = capacityEMOS(HH,SNR)                           %%
%                                                        %%
%  This function was build to calculate the capacity     %%
%  of the estimated EMOS channels. It first normalizes   %%
%  the channel gains, such that E(|h_{i,j}|^2) = 1,      %%
%  where the avaraging is done over all snapshots and    %%
%  all frequencies.                                      %%
...
%  Modified last: 10/10/2007 by florian.kaltenberger@eurecom.fr
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% %%
...
if normalize
    ...
    for ii = 1:N_fq,
        for kk = 1:N_channel,
            % this is the frobenious norm of HH
            var1(:,:,ii,kk) = HH(:,:,ii,kk)*HH(:,:,ii,kk)';
            tmp = tmp + trace(var1(:,:,ii,kk));
        end
    end
    PP = tmp/(N_fq*N_channel);
else
    PP = 1;
end
...
for ii = 1:N_fq,
    for kk = 1:N_channel,
        ...
        % more efficient
        tmp(ii,kk)=abs(log2(det(diag(ones(M_Rx,1)) + (SNR/M_Tx) * HH(:,:,ii,kk) * HH(:,:,ii,kk)')));
