...
% CAP_2U_ZF = zeros(NFreq,NFrames_Tx);
% CAP_2U_MMSE = zeros(NFreq,NFrames_Tx);
...
while any(NFrames_read < sum(align_matrix,2).')
     ...
     for k2=1:size(H_fq,4)
        ...
        for k1=1:size(H_fq,3)
            ...
            for k0=1:length(SNRdB)
                ...
                % [CAP_2U_MMSE(k1,NFrames_Tx_read + k2), gain] = MISO_MMSE(HS,n_p,mu);
                % gain_2U_MMSE(NFrames_Tx_read + k2,:) = gain_2U_MMSE(NFrames_Tx_read + k2,:) + gain;
                % [CAP_2U_ZF(k1,NFrames_Tx_read + k2), gain] = MISO_MMSE(HS,n_p,0);
    ...
    filename_mat = fullfile(filepath,sprintf('results_DPC_%dx1_idx_%d.mat',NTx,idx));
