function [H_MU, H_fq_MU, estimates, gps_data, NFrames] = load_estimates2_MU_lm_new(fid, NTx, NFrames_max, NFrames, align_matrix, version)
% 
...
%
% Returns:
% H                 - Time domain channel matrix of size [NRx,NTx,NDelay,NFrames,Nuser,NCH]
%                     The estimates of different users are aligned in time
% H_fq              - Frequency domain channel matrix of size [NRx,NTx,NFreq,NFrames,Nuser,NCH]
...
H_MU = complex(zeros(NRx,NTx,NDelay,NFrames_max,Nuser,NCH));
H_fq_MU = complex(zeros(NRx,NTx,NFreq,NFrames_max,Nuser,NCH));

for u=1:Nuser
    if (sum(align_matrix(u,:))>0)
        [H_MU(:,:,:,align_matrix(u,:),u,:), ...
            H_fq_MU(:,:,:,align_matrix(u,:),u,:), ...
            estimates(u), gps_data(u), NFrames(u)] = ...
            load_estimates2_lm(fid(u), NTx, ...
            sum(align_matrix(u,:)), ...
            NFrames(u), version);
    end
end
