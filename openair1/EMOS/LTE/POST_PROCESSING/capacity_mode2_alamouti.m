...
chcap_alamouti = zeros(1,1200, length(estimates_UE1));
chcap_alamouti_16 = zeros(1,1200, length(estimates_UE1));
chcap_alamouti_64 = zeros(1,1200, length(estimates_UE1));
...
for est=1:length(estimates_UE1)
    ...
    SNR_eNB1_1stRx = 10*log10(((abs(h11_eNB1_comp).^2) + (abs(h21_eNB1_comp).^2))/N0_1stRx);
    SNR_eNB1_2ndRx = 10*log10(((abs(h12_eNB1_comp).^2) + (abs(h22_eNB1_comp).^2))/N0_2ndRx);
    %SNR_eNB1_2Rx = 10*log10((((abs(h11_eNB1_comp).^2) + (abs(h12_eNB1_comp).^2))/NO_1stRx) + (((abs(h21_eNB1_comp).^2) + (abs(h22_eNB1_comp).^2))/NO_2ndRx));
    
    SNR_eNB2_1stRx = 10*log10(((abs(h11_eNB2_comp).^2) + (abs(h21_eNB2_comp).^2))/N0_1stRx);
    SNR_eNB2_2ndRx = 10*log10(((abs(h12_eNB2_comp).^2) + (abs(h22_eNB2_comp).^2))/N0_2ndRx);
    %SNR_eNB2_2Rx = 10*log10((((abs(h11_eNB2_comp).^2) + (abs(h12_eNB2_comp).^2))/NO_1stRx) + (((abs(h21_eNB2_comp).^2) + (abs(h22_eNB2_comp).^2))/NO_2ndRx));
    ...
    for i=1:1:7
        ...
        if(sum(SNR_eNB2(:)) < sum(SNR_eNB1(:)))
            chcap_alamouti_1 = [chcap_alamouti_1 (interp1(SNR,c_siso_4Qam,SNR_eNB1(:),'nearest','extrap'))'];
            chcap_alamouti_2 = [chcap_alamouti_2 (interp1(SNR,c_siso_16Qam,SNR_eNB1(:),'nearest','extrap'))'];
            chcap_alamouti_3 = [chcap_alamouti_3 (interp1(SNR,c_siso_64Qam,SNR_eNB1(:),'nearest','extrap'))'];
            
        else
            chcap_alamouti_1 = [chcap_alamouti_1 (interp1(SNR,c_siso_4Qam,SNR_eNB2(:),'nearest','extrap'))'];
            chcap_alamouti_2 = [chcap_alamouti_2 (interp1(SNR,c_siso_16Qam,SNR_eNB2(:),'nearest','extrap'))'];
            chcap_alamouti_3 = [chcap_alamouti_3 (interp1(SNR,c_siso_64Qam,SNR_eNB2(:),'nearest','extrap'))'];
        
        end
    end
    
    chcap_alamouti(1,:,est) = chcap_alamouti_1;
    chcap_alamouti_16(1,:,est) = chcap_alamouti_2;
    chcap_alamouti_64(1,:,est) = chcap_alamouti_3;
end