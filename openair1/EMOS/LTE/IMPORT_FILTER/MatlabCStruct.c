...
#define SPACES {int k;for (k=0; k<NestLevel; k++) printf("  "); }
  ...
  case mxCELL_CLASS    :
    break; // Fall down to code below

  case mxSTRUCT_CLASS  :
    break; // Fall down to code below

  case mxCHAR_CLASS    :
    Attributes.ElementSize = sizeof(mxChar);
    break;

  case mxLOGICAL_CLASS :
    Attributes.ElementSize = sizeof(mxLogical);
    break;

  case mxDOUBLE_CLASS  :
    Attributes.ElementSize = sizeof(double);
    break;

  case mxSINGLE_CLASS  :
    Attributes.ElementSize = sizeof(float);
    break;

  case mxINT8_CLASS    :
    Attributes.ElementSize = sizeof(char);
    break;

  case mxUINT8_CLASS   :
    Attributes.ElementSize = sizeof(unsigned char);
    break;

  case mxINT16_CLASS   :
    Attributes.ElementSize = sizeof(short);
    break;

  case mxUINT16_CLASS  :
    Attributes.ElementSize = sizeof(unsigned short);
    break;

  case mxINT32_CLASS   :
    Attributes.ElementSize = sizeof(int);
    break;

  case mxUINT32_CLASS  :
    Attributes.ElementSize = sizeof(unsigned int);
    break;

    //  case mxINT64_CLASS   : Attributes.ElementSize = sizeof(__int64); break;
  case mxINT64_CLASS   :
    Attributes.ElementSize = sizeof(INT64_T);
    break; // jaj 05/24/2004

  default:
    mexErrMsgTxt("Unknown MX class");
  }
