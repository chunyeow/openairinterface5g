...
typedef struct MIH_C_Link_Going_Down_indication {
  MIH_C_LINK_TUPLE_ID_T      LinkIdentifier;                /*!< \brief  Identifier of the link associated with the event. */
  UNSIGNED_INT2(
    TimeInterval)                               /*!< \brief  Time Interval (in milliseconds) specifies the time interval at which the link is expected to go down. A value of "0" is specified if the time interval is unknown.*/
...
typedef struct MIH_C_Link_Capability_Discover_confirm {
  ...
  *SupportedLinkEventList;       /*!< \brief  List of link-layer events supported by the link layer. This parameter is not included if Status does not indicate “Success.” */
  MIH_C_LINK_CMD_LIST_T
  *SupportedLinkCommandList;     /*!< \brief  List of link-layer commands supported by the link layer. This parameter is not included if Status does not indicate “Success.” */
...
typedef struct MIH_C_Link_Action_request {
  ...
  UNSIGNED_INT2(
    ExecutionDelay)                                /*!< \brief  Time (in ms) to elapse before the action needs to be taken. A value of 0 indicates that the action is taken immediately. Time elapsed is calculated from the instance the request arrives until the time when the execution of the action is carried out. */
...
typedef struct MIH_C_Link_Action_confirm {
  MIH_C_STATUS_T             Status;                           /*!< \brief  Status of operation. Code 3 (Authorization Failure) is not applicable.*/
  LIST(MIH_C_LINK_SCAN_RSP,
       *ScanResponseSet)                  /*!< \brief  (Optional) A list of discovered links and related information. This parameter is not included if Status does not indicate “Success.” */
