...
void NW_TMR_CALLBACK(nwUdpDataIndicationCallbackData)
{
  ...
  if(bytesRead)
  {
    ...
    rc = nwGtpv1uProcessUdpReq(thiz->hGtpv1uStack, udpBuf, bytesRead, peer.sin_port, peer.sin_addr.s_addr);
  }
  else
  {
...
NwGtpv1uRcT nwMiniUdpInit(NwMiniUdpEntityT* thiz, NwGtpv1uStackHandleT hGtpv1uStack, NwU8T* ipAddr)
{
  ...
  event_set(&(thiz->ev), sd, EV_READ|EV_PERSIST, nwUdpDataIndicationCallbackData, thiz);
    ...
    NwU32T peerPort)
{
  ...
  NW_LOG(NW_LOG_LEVEL_DEBG, "Sending %u bytes on handle 0x%x to peer %u.%u.%u.%u:%u", dataSize, udpHandle,
