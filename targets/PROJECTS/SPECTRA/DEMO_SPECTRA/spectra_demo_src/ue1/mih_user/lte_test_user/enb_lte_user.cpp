...
//-----------------------------------------------------------------------------
{
    ...
    ss >> tmp >> value;
    std::cout<<"Result of Sensing "<<value<<std::endl;
    sensing_done = 1;
    return value;
}
...
//                 send_MIH_Link_Actions_request(link, odtone::mih::link_ac_type_power_up);
                ...
                if (second_link_activated==0)
                {
                    if (count == 20)
                    {
                    send_MIH_Link_Action_Power_Up_request(_link_id_list[1]);//Link LTE
...
//-----------------------------------------------------------------------------
{
	...
	try {
		...
		desc.add_options()
			...
			(odtone::sap::kConf_MIH_SAP_dest, po::value<std::string>()->default_value("mihf1_enb"), "MIHF destination");
