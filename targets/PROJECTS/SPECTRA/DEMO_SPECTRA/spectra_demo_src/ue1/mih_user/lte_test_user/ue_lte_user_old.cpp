    ...
    if (ntal && evt) {
        ...
        for (odtone::mih::net_type_addr_list::iterator i = ntal->begin(); i != ntal->end(); i++) {
            if (i->nettype.link.which() == 1)
            {
                ...
                mih_user::send_MIH_Event_Subscribe_request(li, evt.get(), m);
            }
        }
    }
...
// 	    if (_last_link_action_type ==  odtone::mih::link_ac_type_power_up )
// 	    {
// 	      send_MIH_Link_Action_Power_Up_plus_scan_request(i->id);
// 	    }
//         }
//     }
//     log_(0, "");
