...
#define NB_OF_RESOURCES     4   // Should not exceed mih_user::_max_link_action_req
//#define SCENARIO_1    // Sequentially activate and deactivate each resou
//#define SCENARIO_2    // Activate all resources, then deactivate all resour
...
// int& operator [](const link_param_lte_enum value)
// {
//     static std::map<link_param_lte_enum, std::string> var;
//     if (var.size() == 0)
//     {
//         var[link_param_lte_rsrp]="link_param_lte_rsrp";
//         var[link_param_lte_rsrq]="link_param_lte_rsrq";
//         var[link_param_lte_cqi]="link_param_lte_cqi";
//         var[link_param_lte_bandwidth]="link_param_lte_bandwidth";
//         var[link_param_lte_pkt_delay]="link_param_lte_pkt_delay";
//     }
//     return var[value];
// }
...
const char * MeasurementTypes[]=
{
    "LTE_RSRP",        /**< RSRP.                      */
    "LTE_RSRQ",        /**< RSRQ. */
    "link_param_lte_cqi",         /**< Multicast packet loss rate.*/
    "link_param_lte_bandwidth",   /**< System Load.                   */
    "link_param_lte_pkt_delay",   /**< Number of registered users.    */
    "link_param_lte_pkt_loss",    /**< Number of active users.        */
    "link_param_lte_l2_buffer",   /**< Congestion windows of users.   */
    ...
    "link_param_lte_link_congestion",    /**< Link congestion.   */
};
...
protected:
    ...
    void forward_Parameters_Report_indication(odtone::mih::message& msg);
...
//-----------------------------------------------------------------------------
{
    ...
    //send a capability discover request to the remote eNB
...
//-----------------------------------------------------------------------------
{
    ...
    case odtone::mih::indication::mn_ho_candidate_query:
        log_(0, "MIH-User has received a request \"mn_ho_candidate_query\"", msg.mid());
    ...
    default:
        log_(0, "MIH-User has received UNKNOWN local event", msg.mid());
        break;
    }
}
...
//MIH_MN_HO_Candidate_Query_request used to forward the parameters report from eNB1 to eNB2
//-----------------------------------------------------------------------------
...
for (odtone::mih::link_param_rpt_list::iterator i=lprl.begin(); i!=lprl.end(); i++)
    {
            ...
            {
                log_(0, "Meausrement Value: ", (short) *value /*- 65536*/);
            }

    }
    log_(0, "MIH_Link_Parameters_Report.indication - End");

    //forward the Link parameters report to eNB2
...
//-----------------------------------------------------------------------------
{
    ...
    m << odtone::mih::request(odtone::mih::request::capability_discover);
    m.source(_mihuserid);
    odtone::mih::id mid_enb;
    mid_enb.assign("mihf4_enb");
    m.destination(mid_enb);
//     m.destination(_mihfid);
...
//   
    ...
    if (link.type == odtone::mih::link_type_lte)
...
//                      "2001:0660:0382:0014:0335:0600:8014:9150");
//  }
//  else if (mih_user::_current_link_action_request == 1) {
//    res.fid.dst.ip = odtone::mih::ip_addr(odtone::mih::ip_addr::ipv6,
//                      "2001:0660:0382:0014:0335:0600:8014:9151");
//  }
//  else if (mih_user::_current_link_action_request == 2) {
//    res.fid.dst.ip = odtone::mih::ip_addr(odtone::mih::ip_addr::ipv6,
//                      "FF3E:0020:2001:0DB8:0000:0000:0000:0043");
//    res.multicast = true;
//  }
//  else if (mih_user::_current_link_action_request == 3) {
//    res.fid.dst.ip = odtone::mih::ip_addr(odtone::mih::ip_addr::ipv6,
//                      "2001:0660:0382:0014:0335:0600:8014:9153");
//  }
...
void mih_user::forward_Parameters_Report_indication(odtone::mih::message& m)
{
    ...
    log_(0, "Forward_Parameters_Report.indication - Start");
   ...
   log_(0, "[MSC_MSG] ["+getTimeStamp4Log()+"]["+msg.source().to_string()+"][-------- Forward MIH_User_Parameters.indication ---->]["+msg.destination().to_string()+"]\n");
   
   _mihf.async_send(msg, boost::bind(&mih_user::event_handler, this, _1, _2));
      log_(0, "Forward_Parameters_Report.indication - End");
}
...
//-----------------------------------------------------------------------------
{
    ...
    odtone::mih::threshold               th3;
    std::vector<odtone::mih::threshold>  thl3;
    ...
    odtone::mih::link_cfg_param          lcp3;
    odtone::mih::link_param_lte          lp1;
    odtone::mih::link_param_lte          lp2;
    odtone::mih::link_param_lte          lp3;
    ...
    lp3 = odtone::mih::link_param_lte_cqi;
    lcp1.type = lp1;
    lcp2.type = lp2;
    lcp3.type = lp3;
    ...
    th3.threshold_val = 0;
    th3.threshold_x_dir = odtone::mih::threshold::above_threshold;
    ...
    if(lp2 == odtone::mih::link_param_lte_rsrq)       {log_(0, "\t  LTE link parameter LTE RSRQ");}
...
//-----------------------------------------------------------------------------
{
    ...
    for(iter=0; iter<lcsl.size(); iter++)
    {
        ...
        log_(0, "\t  Threshold Val: ", ((short) lcsl[iter].thold.threshold_val));
...
//-----------------------------------------------------------------------------
{
    ...
    try {
        ...
        desc.add_options()
            ...
            (odtone::sap::kConf_File, po::value<std::string>()->default_value("ue2_user.conf"), "Configuration file")
            ...
            (odtone::sap::kConf_MIH_SAP_dest, po::value<std::string>()->default_value("mihf3_ue"), "MIHF destination");
