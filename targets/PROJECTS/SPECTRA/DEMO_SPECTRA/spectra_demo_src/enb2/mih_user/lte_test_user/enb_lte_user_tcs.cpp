...
//           Fatma HRIZI EURECOM <hrizi@eurecom>fr>
//------------------------------------------------------------------------------
...
//This file is the implementation of the MIH user in the eNB
...
protected:
  ...
  void send_MIH_Capability_Discover_request(const odtone::mih::config& cfg);
...
//-----------------------------------------------------------------------------
{
    ...
    //
    // Local Capability Discover Request
    //
    odtone::mih::message msg;
    _mihfid.assign("mihf2");
    ...
    _mihf.async_send(msg, boost::bind(&mih_user::receive_MIH_Capability_Discover_confirm, this, _1));
    log_(0, "MIH_Capability_Discover.request - SENT (towards its local MIHF)");
    log_(0, "");

  //****eNB*****
  // Remote Capability Discover Request 
  //
   mih_user::send_MIH_Capability_Discover_request(cfg);
   
   //Trigger Link_Configure_Thresholds Request
...
boost::system::error_code ec;
          for (odtone::mih::link_det_info_list::iterator i = ldil.begin(); i != ldil.end(); i++)
          {
		log_(0, "   - LINK_ID - Link identifier:  ", link_id2string(i->id).c_str());
		log_(0, "   - Network ID:  ", i->network_id);
		log_(0, "   - SINR:  ", i->sinr);
		log_(0, "   - Data_rate:  ", i->data_rate);		
        }

	send_MIH_Link_Configure_Thresholds_request(msg, ec);
...
//-----------------------------------------------------------------------------
{
    ...
    log_(0, "  - LINK_TUPLE_ID - Link identifier:  ", link_id2string(link).c_str());
	  ...
	  {
		  if (const odtone::mih::threshold *th = boost::get<odtone::mih::threshold>(&i->thold)) {
			...
			log_(0, "   -Regular Report for BW Threshold ");
			//update MEAS	
		    }
	}
...
//-----------------------------------------------------------------------------
void mih_user::send_MIH_Capability_Discover_request(const odtone::mih::config& cfg)
//-----------------------------------------------------------------------------
{
  odtone::mih::message m;
  ...
  log_(0, "MIH_Capability_Discover.request - SENT\n");
}
...
//-----------------------------------------------------------------------------
{
  ...
  try {
    ...
    desc.add_options()
      ...
      (odtone::sap::kConf_File, po::value<std::string>()->default_value("lte_user.conf"), "Configuration file")
