...
//The thresholds for the Energy Detection Sensing algorithm respectively for 10 and 100 samples
#define ED_THRESHOLD_10 23695432
#define ED_THRESHOLD_100 230445932
///////////////////////////////////////////////////////////////////////////////
// The scenario coded in this MIH-USER (of the eNB) for the demo of the Scenario 2 of SPECTRA project is the following
//  +--------+                                             +-----+                                             +---------+
//  |MIH_USER|                                             |MIH-F|                                             |LINK_SAP |
//  +---+----+                                             +--+--+                                             +----+----+
//      |                                                     |                                                     |
// ------------------------------------------------------------------------------------------------------------------------
//                            Initiallization of the MIH-USER and the MIHF
// ------------------------------------------------------------------------------------------------------------------------
//      |                                                     |                                                     |
//     ...          (start of MIH-F here)                    ...                                                   ...
...
// ------------------------------------------------------------------------------------------------------------------------
//                            Locally send the Capability Discover and the Event Subscribe primitives
//                                                  (from MIH USER to local MIHF)
// ------------------------------------------------------------------------------------------------------------------------
//      |                                                     |                                                     |
...
// ------------------------------------------------------------------------------------------------------------------------
//                            Remotely send the Capability Discover and the Event Subscribe primitives
//                                            (from MIH USER to remote MIHF of the UE)
// ------------------------------------------------------------------------------------------------------------------------
//      |                                                     |                                                     |
//      |---------- MIH_Capability_Discover.request --------->|                                                     |
//      |<--------- MIH_Capability_Discover.confirm ----------|                                                     |
...
//      |                                                     |                                                     |
// ------------------------------------------------------------------------------------------------------------------------
//                            Detect the connection of the UE to the eNB + Start the measurement report process
// ------------------------------------------------------------------------------------------------------------------------
//      |                                                     |                                                     |
//      |                                                     |                                                     |
...
//      |                                                     |                                                     |
// ------------------------------------------------------------------------------------------------------------------------
//                            Activate the TVWS link (After running the cognitive algorithms)
// ------------------------------------------------------------------------------------------------------------------------
//      |                                                     |                                                     |
//      |-------------- MIH_Link_Actions.request ------------>|--------------- MIH_Link_Actions.request ----------->|
//      |                                                     |                                                     |
//      |<------------- MIH_Link_Actions.confirm -------------|<-------------- MIH_Link_Actions.confirm ------------|
//      |                                                     |                                                     |
...
protected:
	...
	void send_MIH_Capability_Discover_request_remote(void);
	void receive_MIH_Capability_Discover_confirm(odtone::mih::message& msg);

	void send_MIH_Event_Subscribe_request(odtone::mih::link_tuple_id& li, odtone::mih::mih_evt_list& evt, odtone::mih::id dest);
	...
	void forward_Parameters_Report_indication(odtone::mih::message& m);
	
	void receive_MIH_MN_HO_Candidate_Query_request(odtone::mih::message& msg);
...
private:
    ...
    odtone::uint second_link_activated;
        odtone::uint count;
    odtone::uint sensing_done;
    odtone::uint sensing_score;
...
//-----------------------------------------------------------------------------
{
	...
	//send a capability discover request to the remote UE
	mih_user::send_MIH_Capability_Discover_request_remote();
}
...
//-----------------------------------------------------------------------------
{
	...
	case odtone::mih::indication::mn_ho_candidate_query:
		log_(0, "MIH-User has received a request \"mn_ho_candidate_query\"");
		mih_user::receive_MIH_MN_HO_Candidate_Query_request(msg);
		break;
...
//-----------------------------------------------------------------------------
void mih_user::send_MIH_Capability_Discover_request_remote(void)
//-----------------------------------------------------------------------------
{
	odtone::mih::message m;
	m << odtone::mih::request(odtone::mih::request::capability_discover);
	m.source(_mihuserid);
	odtone::mih::id mid_ue;
	mid_ue.assign("mihf3_ue");
	m.destination(mid_ue);

	log_(0, "[MSC_MSG]["+getTimeStamp4Log()+"]["+ m.source().to_string() +"][--- MIH_Capability_Discover.request --->]["+m.destination().to_string()+"]\n");

	_mihf.async_send(m, boost::bind(&mih_user::receive_handler, this, _1, _2));

	log_(0, "MIH_Capability_Discover.request - SENT (towards the remote MIHF)\n");
}
...
//           }
	...
	if (ntal && evt) {
        ...
        std::cout<<"NTALL  "<<ntal.get()[0].addr<<std::endl;
		for (odtone::mih::net_type_addr_list::iterator i = ntal->begin(); i != ntal->end(); i++) {
...
//                 stream << net_type_addr;
//                 std::string s;
//                 std::stringstream st (stream.str());
//                 while (getline (st, s, '\n'))
//                 {
...
/*
                st<<i->addr;
                getline (st, s, ' ');
                std::cout<<"LINK_TUPLE_ID s "<<s<<std::endl;
                
                odtone::mih::l2_3gpp_addr add;
                add.value = s;*/
// 			    li.addr = /*add*/ boost::get<odtone::mih::l2_3gpp_addr>(i->addr);
// 			    li.type = boost::get<odtone::mih::link_type>(i->nettype.link);
//                 if (std::find(_link_id_list.begin(), _link_id_list.end(), li)==_link_id_list.end())
// 			    _link_id_list.push_back(li);	// save the link identifier of the network interface
                ...
                std::cout<<"LINK_TUPLE_ID - Link identifier:after "<<link_addr2string(&li.addr).c_str() <<"  "<<_link_id_list[_link_id_list.size()-1]<<std::endl;
			    mih_user::send_MIH_Event_Subscribe_request(li, evt.get(), msg.source());
			}
		}
	}
...
//-----------------------------------------------------------------------------
void mih_user::send_MIH_Event_Subscribe_request(odtone::mih::link_tuple_id& li, odtone::mih::mih_evt_list& evt, odtone::mih::id dest)
//-----------------------------------------------------------------------------
{
	odtone::mih::message m;
    ...
    std::cout<<"LINK_TUPLE_ID - Link identifier:  "<<li<<std::endl;
...
//-----------------------------------------------------------------------------
void mih_user::send_MIH_Link_Action_Power_Up_request(const odtone::mih::link_id& link)
//-----------------------------------------------------------------------------
{
    odtone::mih::message          m;
...
//             for (odtone::mih::link_id_list::iterator i = _link_id_list.begin(); i != _link_id_list.end(); i++) {
//             std::cout<<"_link_id_list  addr "<<i->addr<<" TYPE "<<i->type<<std::endl;
...
//         }
    ...
    std::cout<<"_link received from parameters report "<<link<<std::endl;
        for (odtone::mih::link_id_list::iterator i = _link_id_list.begin(); i != _link_id_list.end(); i++) {
            std::cout<<"_link_id_list  addr "<<i->addr<<" TYPE "<<i->type<<std::endl;
        }
...
//     odtone::mih::id mid_ue;
//     mid_ue.assign("mihf2_ue");
	m.destination(/*mid_ue*/_mihfid);
...
// {
// 
//     //Sensing measurements report
//     std::string result = exec ("./client 127.0.0.1 4546 10 | grep \"SCORE\"");
// }


void mih_user::receive_CRRM_Data()
{

    //CRRM data report
    std::string result = exec ("./CRMClientmain 0 1");
}


//-----------------------------------------------------------------------------
int mih_user::receive_Sensing_Report()
//-----------------------------------------------------------------------------
{
    std::string result = exec ("./client 127.0.0.1 4546 10 | grep \"SCORE\"");
    std::string tmp;
...
void mih_user::receive_MIH_MN_HO_Candidate_Query_request(odtone::mih::message& msg)
//-----------------------------------------------------------------------------
{
    odtone::mih::link_tuple_id link;
...
//eNB2 : Action Power Up the TVWS Link after running the cognitive algorithm
    ...
    //Sensing data
     if (sensing_done == 0)
     sensing_score = receive_Sensing_Report();
    ...
    // receive_CRRM_Data();
          ...
          for (odtone::mih::link_param_rpt_list::iterator i = lprl.begin(); i != lprl.end(); i++)
            {
                if (odtone::mih::threshold *th = boost::get<odtone::mih::threshold>(&i->thold)) {
//                 log_(0, "   - BW Threshold crossed, Value:", boost::get<odtone::mih::link_param_val>(i->param.value));
                //link_action if free channel is available
//                 for (odtone::mih::link_id_list::iterator i = _link_id_list.begin(); i != _link_id_list.end(); i++) {
//                 if(*i==link)
                odtone::mih::l2_3gpp_addr add;
                odtone::mih::link_type type;
                type = odtone::mih::link_type_lte;
//                 add.value = "l2_3gpp_addr";
//                 link1.addr = add;
//                 link1.type = type;
...
//               		 log_(0, "COUNT: ", count );
                ...
                count++;
//                     continue;
//                 send_MIH_Link_Action_Power_Up_request(link);
//                     if (link_id2string(link).c_str() == "LTE")
//             }
            }
     }
...
//-----------------------------------------------------------------------------
{
    odtone::mih::link_tuple_id link, link1;
    odtone::mih::link_param_rpt_list lprl;
...
//                 send_MIH_Link_Actions_request(link, odtone::mih::link_ac_type_power_up);
                ...
                //Send ONLY ONCE RRC_Connection_Reconfiguration request to the UE on LTE link 0
                 if (second_link_activated==0)
                 {
                 send_MIH_Link_Action_Power_Up_request(_link_id_list[0]);
                 second_link_activated = 1;
                 }
//                     continue;
  //               send_MIH_Link_Action_Power_Up_request(link);
...
//Forward the MIH_Link_Parameters_Report to the UE of the CPE
//-----------------------------------------------------------------------------
void mih_user::forward_Parameters_Report_indication(odtone::mih::message& m)
//-----------------------------------------------------------------------------
{
    odtone::mih::link_tuple_id link;
    odtone::mih::link_param_rpt_list lprl;

    m >> odtone::mih::indication()
           & odtone::mih::tlv_link_identifier(link)
           & odtone::mih::tlv_link_param_rpt_list(lprl);

    odtone::mih::message  msg;
    msg.source(_mihuserid);
    odtone::mih::id mid_ue;
    mid_ue.assign("mihf3_ue");
    msg.destination(mid_ue);

    //mn_ho_candidate_query is used to constuct/send the message containing parameters reports
    msg <<  odtone::mih::request(odtone::mih::request::mn_ho_candidate_query)
          & odtone::mih::tlv_link_identifier(link)
          & odtone::mih::tlv_link_param_rpt_list(lprl);
  
	log_(0, "[MSC_MSG]["+getTimeStamp4Log()+"]["+ msg.source().to_string() +"][--- before MIH_User_PARAMETERS.indication --->]["+msg.destination().to_string()+"]\n");


    _mihf.async_send(msg, boost::bind(&mih_user::event_handler, this, _1, _2));

	log_(0, "MIH_User_PARAMETERS.indication- SENT (towards UE)\n");
...
{
    odtone::mih::message                 m;
    odtone::mih::threshold               th1;
    odtone::mih::threshold               th2;
    std::vector<odtone::mih::threshold>  thl1;
    std::vector<odtone::mih::threshold>  thl2;
    ...
    odtone::mih::link_cfg_param          lcp1;
    odtone::mih::link_cfg_param          lcp2;
    odtone::mih::link_param_lte          lp1;
    odtone::mih::link_param_lte          lp2;
    ...
    lp1 = odtone::mih::link_param_lte_rsrp;
    lp2 = odtone::mih::link_param_lte_rsrq;
    lcp1.type = lp1;
    lcp2.type = lp2;
    ...
    lcp1.timer_interval = 3000;
    lcp2.timer_interval = 3000;
    //th_action_normal = 0,   /**< Set normal threshold.      */
    //th_action_one_shot = 1, /**< Set one-shot threshold.    */
    //th_action_cancel = 2    /**< Cancel threshold.          */
    lcp1.action = odtone::mih::th_action_normal;
    lcp2.action = odtone::mih::th_action_normal;
    link_measures_request = 1;
    } else if ( link_measures_request==1){
    // Set Timer Interval (in ms)
    lcp1.timer_interval = 0;
    lcp2.timer_interval = 0;
    lcp1.action = odtone::mih::th_action_cancel;
    ...
    th1.threshold_val = -105;
    th2.threshold_val = -19;
    th1.threshold_x_dir = odtone::mih::threshold::above_threshold;
    th2.threshold_x_dir = odtone::mih::threshold::above_threshold;

    thl1.push_back(th1);
    thl2.push_back(th2);
    lcp1.threshold_list = thl1;
    lcp2.threshold_list = thl2;
    ...
    m.source(_mihuserid);
    odtone::mih::id mid_ue;
    mid_ue.assign("mihf3_ue");//mihf2_ue
    m.destination(mid_ue);
    ...
    log_(0, "[MSC_MSG]["+getTimeStamp4Log()+"]["+ _mihuserid.to_string() +"][--- MIH_Link_Configure_Thresholds.request\\nlink="+
	        ...
	        link_id2string(lti).c_str()+ " --->][  ADDR LINK "
            + m.destination().to_string() +"]\n");
    std::cout<<"LINK TUPLE ID "<<lti<<std::endl;
    ...
    if(lp1 == odtone::mih::link_param_lte_rsrp)       {log_(0, "\t  LTE link parameter LTE RSRP");}

    log_(0, "\t- TIMER INTERVAL - Value: ", lcp1.timer_interval);

    if(lcp1.action == odtone::mih::th_action_normal) {log_(0, "\t  Normal Threshold");}
    if(lcp1.action == odtone::mih::th_action_one_shot) {log_(0, "\t  One Shot Threshold");}
    if(lcp1.action == odtone::mih::th_action_cancel) {log_(0, "\t  Threshold to be canceled");}

    log_(0, "\t  Threshold value: ", (short) th1.threshold_val);

    if(th1.threshold_x_dir == odtone::mih::threshold::below_threshold) {log_(0, "\t  Threshold direction BELOW");}
    if(th1.threshold_x_dir == odtone::mih::threshold::above_threshold) {log_(0, "\t  Threshold direction ABOVE");}
...
//         & odtone::mih::tlv_link_cfg_status_list(lcsl);
    ...
    log_(0, "[MSC_MSG]["+getTimeStamp4Log()+"]["+ msg.source().to_string() +"][--- MIH_Link_Configure_Thresholds.confirm\\nstatus="+status2string(st.get()).c_str()+" --->]["+_mihuserid.to_string()+"]\n");
    ...
    {
        log_(0, "\t  Link Param Type: ", lcsl[iter].type);
        log_(0, "\t  Threshold Val: ", (lcsl[iter].thold.threshold_val/256));
        if(lcsl[iter].thold.threshold_x_dir == odtone::mih::threshold::below_threshold) {log_(0, "\t  Threshold direction BELOW");}
        if(lcsl[iter].thold.threshold_x_dir == odtone::mih::threshold::above_threshold) {log_(0, "\t  Threshold direction ABOVE");}
        if(lcsl[iter].status == odtone::mih::status_success){log_(0, "\t  Config Status: Success ");}
        else {log_(0, "\t  Config Status: ", lcsl[iter].status);}
    }
...
//-----------------------------------------------------------------------------
{
	...
	try {
		...
		desc.add_options()
			...
			(odtone::sap::kConf_File, po::value<std::string>()->default_value("enb_lte_user.conf"), "Configuration file")
			(odtone::sap::kConf_Receive_Buffer_Len, po::value<uint>()->default_value(4096), "Receive buffer length")
			(odtone::sap::kConf_Port, po::value<ushort>()->default_value(1635), "Listening port")
			...
			(odtone::sap::kConf_MIH_SAP_dest, po::value<std::string>()->default_value("mihf4_enb"), "MIHF destination");
